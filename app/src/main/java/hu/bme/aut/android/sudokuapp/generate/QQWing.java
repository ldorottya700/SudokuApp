...
public class QQWing {
	...
	public Difficulty getDifficulty() {
		if (getGuessCount() > 0) return Difficulty.HARD;
		if (getBoxLineReductionCount() > 0) return Difficulty.MEDIUM;
		if (getPointingPairTripleCount() > 0) return Difficulty.MEDIUM;
		if (getHiddenPairCount() > 0) return Difficulty.MEDIUM;
		if (getNakedPairCount() > 0) return Difficulty.MEDIUM;
